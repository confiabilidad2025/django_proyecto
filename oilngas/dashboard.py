...
def update_production_text(well_statuses, well_types, year_slider):
    dff = utils.filter_dataframe(
        oildata.dataframe, well_statuses, well_types, year_slider
    )
    selected = dff["API_WellNo"].values
    index, gas, oil, water = utils.fetch_aggregate(
        oildata.points, selected, year_slider
    )
    return [
        utils.human_format(sum(gas)),
        utils.human_format(sum(oil)),
        utils.human_format(sum(water)),
    ]
...
@dashboard.callback(Output("well_types", "value"), Input("well_type_selector", "value"))
...
@dashboard.callback(Output("gasText", "children"), Input("aggregate_data", "data"))
...
@dashboard.callback(Output("oilText", "children"), Input("aggregate_data", "data"))
...
@dashboard.callback(Output("waterText", "children"), Input("aggregate_data", "data"))
...
def make_count_figure(well_statuses, well_types, year_slider):
    dff = utils.filter_dataframe(
        oildata.dataframe, well_statuses, well_types, [1960, 2017]
    )
...
def make_individual_figure(main_graph_hover):
    ...
    gas_data = {
        ...
        "x": index,
        "y": gas,
        "line": {"shape": "spline", "smoothing": 2, "width": 1, "color": "#fac1b7"},
    ...
    oil_data = {
        ...
        "x": index,
        "y": oil,
        "line": {"shape": "spline", "smoothing": 2, "width": 1, "color": "#a9bb95"},
        "marker": {"symbol": "diamond-open"},
    }
    ...
    water_data = {
        ...
        "x": index,
        "y": water,
        "line": {"shape": "spline", "smoothing": 2, "width": 1, "color": "#92d8d8"},
        "marker": {"symbol": "diamond-open"},
    }
...
def make_pie_figure(well_statuses, well_types, year_slider):
    ...
    data = [
        ...
        dict(
            ...
            textinfo="label+percent+name",
            hole=0.5,
            marker=dict(colors=[options.WELL_COLORS[i] for i in aggregate.index]),
    ...
    return {
        ...
        "layout": {
            **commont_graph_layout,
            "title": "Production Summary: {} to {}".format(
