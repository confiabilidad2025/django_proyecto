...
class DjangoStatsDashboard(Dashboard):
                ...
                material.InnerRow(
                    [
                        material.Span4(
                            [
                                self.badge(
                                    "Users registered", "person", self.users_count()
                                )
                            ]
                        ),
                        material.Span4(
                            [self.badge("Groups", "group", self.groups_count())]
                        ),
                        material.Span4(
                            [
                                self.badge(
    ...
    def today_admin_actions(self):
        return LogEntry.objects.filter(action_time__gte=timezone.now().date()).count()

    def login_stats_figure(self):
        today = timezone.now().date()
        data = {today + datetime.timedelta(days=day): 0 for day in range(-5, 0)}
        ...
        return {
            ...
            "layout": {
                ...
                "xaxis": {"tickformat": "%Y-%m-%d", "nticks": len(data) + 1},
                "yaxis": {
                    "dtick": 1,
                },
            },
    ...
    def registration_stats_figure(self):
        stats = (
            User.objects.annotate(label=TruncMonth("date_joined"))
            .values("label")
